...
(fn [{:keys [db]} [_ search-param]]
 {:http-xhrio {:method         :get
                  :uri          (str "https://api.themoviedb.org/3/search/movie?api_key=5dcf7f28a88be0edc01bbbde06f024ab&language=en-US&query=" (get search-param :query) "&page=1&include_adult=false")
                  :timeout       8000 
                  :response-format (ajax/json-response-format {:keywords? true})  ;; IMPORTANT!: You must provide this.
                  :on-success      [:success-http-result]
                  :on-failure      [:error-message "Failed to load movies"]}
  :db (assoc db :query (get search-param :query))}))